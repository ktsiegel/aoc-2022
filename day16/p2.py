...
def main(filename):
    ...
    while len(q) > 0:
        ...
        memoized_open = (curr[0][0], curr[0][1], curr[1], len(curr[3]))
        ...
        next_time = curr[1]-1
        flow_rate1 = maze[curr[0][0]][0]
        flow_rate2 = maze[curr[0][1]][0]

        # open valve is only an option if the flow > 0 and the valve is not already open
        if flow_rate1 > 0 and curr[0] not in curr[3]:
            next_pressure = curr[2] + (next_time * flow_rate1)
            q.append(
